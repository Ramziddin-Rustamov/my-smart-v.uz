...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Comfortaa:wght@300..700&family=Quicksand:wght@300..700&display=swap"
        rel="stylesheet">
...
<body>
    ...
    <header id="header" class="fixed-top "
        style="background-color: rgba(255, 244, 239, 0.8);
                ...
                <nav id="navbar" class="navbar order-last order-lg-0">
                    <ul>
                        <li><a href="{{ route('team.index') }}"
                                class="{{ request()->routeIs('team.index') ? 'active' : '' }}">Ishchi jamoa</a></li>
                        {{-- <li><a href="{{ route('about') }}" class="{{ request()->routeIs('about') ? 'active' : '' }}">Biz haqimizda</a></li> --}}
                        <li><a href="{{ route('posts.allposts') }}"
                                class="{{ request()->routeIs('posts.allposts') ? 'active' : '' }}">Yangiliklar</a></li>
            ...
            <div class="header-social-links ps-2 d-flex">
                <ul class="navbar-nav ms-auto d-md-flex d-lg-flex ">
                    ...
                    @guest
                        @if (Route::has('login'))
                            <li class="nav-item ">
                                <a style="{{ Request::is('login') ? 'color: green; text-decoration: underline;' : '' }}"
                                    class="nav-link " href="{{ route('login') }}"><span>{{ __('Kirish') }}</span></a>
                            </li>
                        @endif

                        @if (Route::has('register'))
                            <li class="nav-item active">
                                <a style="{{ Request::is('register') ? 'color: green; text-decoration: underline;' : '' }}"
                                    class="nav-link {{ Request::is('register') ? 'active' : '' }}"
                                    href="{{ route('register') }}"><span>{{ __('R. O\'tish') }}</span></a>
                            </li>
                        @endif
                    @else
                        <li class="nav-item dropdown d-flex " id="navbarDropdown">
                            ...
                            <a style="padding-top:16px" id="navbarDropdown" class="nav-link dropdown-toggle" href="#"
                                role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>
                                {{ Auth::user()->first_name }}
                            </a>

                            <div class="dropdown-menu dropdown-menu-end" aria-labelledby="navbarDropdown"
                                style="background-color: rgba(255, 244, 239, 1);
                    box-shadow: 11px 11px 35px -10px rgba(66, 68, 90, 1);">
                                {{-- Main menu --}}
                                ...
                                {{-- My Profile --}}
                                <a class="dropdown-item py-3 {{ request()->routeIs('profile.index') ? 'active' : '' }}"
                                    href="{{ route('profile.index') }}">
                                    <i class="fas fa-user-tie text-success"></i> {{ __(' Mening profilm ') }}
                                </a>

                                @can('shop-owner')
                                    <a class="dropdown-item py-3 {{ request()->routeIs('shops.index') ? 'active' : '' }}"
                                        href="{{ route('shops.index') }}">
                                        <i class="fas fa-store text-success"></i> {{ __('Do`konlarim') }}
                                    </a>
                                @endcan
                                <a class="dropdown-item py-3 {{ request()->routeIs('public.shops.index') ? 'active' : '' }}"
                                    href="{{ route('public.shops.index') }}">
                                    <i class="fas fa-store text-success"></i> {{ __('Do`konlar') }}
                                </a>

                                <a class="dropdown-item py-3 {{ request()->routeIs('announcements.index') ? 'active' : '' }}"
                                    href="{{ route('announcements.index') }}">
                                    <i class="fas fa-bullhorn text-success"></i> {{ __('E`lonlarim ') }}
                                </a>

                                <a class="dropdown-item py-3 {{ request()->routeIs('public.announcements.index') ? 'active' : '' }}"
                                ...
                                <a class="dropdown-item py-3 {{ request()->routeIs('people.index') ? 'active' : '' }}"
                                    href="{{ route('people.index') }}">
                                    <i class="fas fa-user-group text-success"></i> {{ __('Foydalanuvchilar') }}
                                </a>

                                <a class="dropdown-item py-3 {{ request()->routeIs('youth.index') ? 'active' : '' }}"
                                    href="{{ route('youth.index') }}">
                                    <i class="fas fa-users text-success"></i> {{ __(' Yoshlar ') }}
                                </a>

                                <a class="dropdown-item py-3 {{ request()->routeIs('emergency.index') ? 'active' : '' }}"
                                    href="{{ route('emergency.index') }}">
                                    <i class="fas fa-hospital text-success"></i> {{ __('Shoshilinch Telefon') }}
                                </a>

                                <a class="dropdown-item py-3 {{ request()->routeIs('pray.index') ? 'active' : '' }}"
                                    href="{{ route('pray.index') }}">
                                ...
                                {{-- Dashboard --}}
                                @can('super-admin')
                                    <a class="dropdown-item py-3 {{ request()->routeIs('home') ? 'active' : '' }}"
                                        href="{{ route('home') }}">
                                ...
                                <a class="dropdown-item py-3 {{ request()->routeIs('logout') ? 'active' : '' }}"
                                    href="{{ route('logout') }}"
            ...
            <div class="row">
                <div class="col-lg-12">
                    <p class="text-center mb-0  text-white">
                        Sayt 2024 yil boshida ishlab chiqildi. Agarda sayt ishlash jarayomida xato va kamchiliklar
                        bo'lsa xabar bering, bog'lanish uchun
                        <a rel="nofollow" class="text-primary" href="https://t.me/contactmewitherror">Shu yerga
                            bosing</a>
                    </p>
                </div>
            </div>
