            ...
            @foreach ($products as $product)
                <div class="col-md-4 mb-4">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <h5 class="card-title"><span class="text-info h5">Maxsulot nomi:
                                    <br></span>{{ $product->name }}</h5>
                            <p class="card-text"><i class="fa-solid text-primary fa-money-bill"></i>
                                {{ $product->price }}</p>
                            <p class="card-text"><i class="fas fa-map text-primary"></i> {{ $product->shop->address }}
                            </p>
                            <p class="card-text"><i class="fas fa-shop text-primary"></i> {{ $product->shop->name }}</p>
                            <h6>Maxsulot haqida</h6>
                            <div class="col">
                                <p>{{ strlen($product->body) > 100 ? substr($product->body, 0, 100) . '...' : $product->body }}
                                </p>
                            </div>
                        </div>
                    </div>
