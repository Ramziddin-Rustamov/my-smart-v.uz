                ...
                @foreach ($products as $product)
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <h5 class="card-title"><span class="text-info">Maxsulot nomi:
                                        <br></span>{{ $product->name }}</h5>
                                <p class="card-text"><i class="fa-solid text-primary fa-money-bill"></i>
                                    {{ $product->price }}</p>
                                <p class="card-text"> <i class="fas fa-shop text-primary"></i> {{ $product->shop->name }}
                                </p>
                                ...
                                <div class="d-grid gap-2">
                                    ...
                                    <a href="{{ route('products.edit', $product->id) }}"
                                        class="btn btn-secondary">Taxrirlash </a>
                                    <form action="{{ route('products.destroy', $product->id) }}" method="POST"
                                        style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger"
                                            onclick="return confirm('Haqiqatdan ham o`chirmoqchimisiz ?')">O'chirish</button>
                                    </form>
                                </div>
                            </div>
                        </div>
