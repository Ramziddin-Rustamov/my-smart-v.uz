            ...
            <div class="card-body">
                <div class="px-2">
                    ...
                    <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <select class="form-control" id="shop_id" name="shop_id">
                                @foreach ($shops as $shop)
                                    <option value="{{ $shop->id }}"
                                        {{ old('shop_id', isset($product) ? $product->shop_id : '') == $shop->id ? 'selected' : '' }}>
                                        {{ $shop->name }}ga qo'shish</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <label for="image" class="form-label">Rasmini yulang [ majburiy emas yuklamasangiz ham
                                bo'ladi ]</label>
