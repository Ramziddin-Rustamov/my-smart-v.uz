                ...
                <div class="col-lg-12 entries">
                    ...
                    <article class="entry entry-single">
                        ...
                        <div class="entry-img">
                            <a href="{{ asset($post->image) }}">
                                <img style="width: 100%;
                                     object-fit: cover;
                                     border-radius: 1%;" src="{{ asset($post->image) }}" alt="Postrasining rasm" class="img-fluid">
                            </a>
                        </div>
                        ...
                        <div class="entry-meta">
                            <ul>
                                <li class="d-flex align-items-center"><i class="bi bi-clock"></i> <time >{{ $post->created_at }}</time></li>
                                <li class="d-flex align-items-center"><i class="fas fa-message"></i> {{ $post->comments->count() }}</li>
                            </ul>
                        </div>
                        ...
                        <div class="entry-footer d-flex">
                            ...
                            @else
                                ...
                                <form action="{{ route('posts.like', $post->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button  class="btn btn-outline-danger ms-1" type="submit">O'chirish <i class="fa fa-heart ms-1"></i></button><span class="text-green ps-2 font-weight-bold">{{ $post->likes->count() }}{{ Str::plural('like', $post->likes->count()) }}</span>
                                </form>
                            @endif
                        </div>
                        @endauth
                    ...
                    <div class="reply-form mt-4">
                        ...
                        <form method="POST" action="{{ route('posts.comments.store', ['post' => $post->id]) }}">
                            ...
                            @endauth
                            @guest
                                <p>{{ __('Izoh qoldirish uchun') }} <a href="{{ route('register') }}">{{ __('ro`yxatdan o`ting') }}</a> yoki </p>
                    ...
                    @endif
                        @if ($post->comments->count())
                                    ...
                                    <div class="d-flex">
                                        <div class="comment-img me-3" style="width: 35px">
                                            <a href="{{ asset($comment->user->image) }}">
                                                <img class="img-account-profile rounded-circle mb-2" src="{{ asset($comment->user->image)  }}" alt="{{ $comment->user->name }}`s rasmi">
                                            </a>
                                        </div>
                                        <div>
                            ...
                            @endforeach
                        @else
                            <h6 class="text-center">{{ __('Hozircha izohlar mavjud emas') }}</h6><hr />
                        @endif
