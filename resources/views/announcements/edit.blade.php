                        ...
                        <form action="{{ route('announcements.update', $announcement->id) }}" method="POST"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('name') is-invalid @enderror"
                                    id="name" name="name" value="{{ old('name', $announcement->name) }}" required>
                                @error('name')
                            ...
                            <div class="form-group">
                                ...
                                @if ($announcement->photo)
                                    <h4 class="my-3">Avvalgi Rasmi:</h3>
                                        <img src="{{ asset($announcement->photo) }}" alt="{{ $announcement->name }}"
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control @error('is_active') is-invalid @enderror" id="is_active"
                                    name="is_active">
                                    <option value="1"
                                        {{ old('is_active', $announcement->is_active) ? 'selected' : '' }}>Aktive qilish
                                    </option>
                                    <option value="0"
                                        {{ !old('is_active', $announcement->is_active) ? 'selected' : '' }}>Ko'rinmas qilish
                                        !</option>
                                </select>
                                @error('is_active')
                                    <span class="invalid-feedback" role="alert">
